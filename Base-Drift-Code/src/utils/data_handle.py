...
def read_bits_from_str(string, segment_length=120, need_log=False):
    monitor = Monitor()
...
def write_bits_to_str(matrix, bit_size=None, need_log=False):
    if need_log:
    ...
    for index in range(0, len(temp_list), 8):
        byte_str = temp_list[index:index + 8]
        if len(byte_str) < 8:
            break  # 避免尾部不足8位导致错误
        values.append(int("".join(map(str, byte_str)), 2))

    return bytes(values).decode("utf8", errors="ignore")
...
def read_bits_from_file(path, segment_length=120, need_log=True):
    monitor = Monitor()
    if need_log:
...
def write_bits_to_file(path, matrix, bit_size, need_log=True):
    monitor = Monitor()
...
def read_dna_file(path, need_log=True):
    monitor = Monitor()
